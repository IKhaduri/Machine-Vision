...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
        ...
        for x=1:num_histograms
            for y=1:num_histograms
                X1 = (x-1)*pixelsPerHistogram+1;
                Y1 = x*pixelsPerHistogram;
                X2 = (y-1)*pixelsPerHistogram+1;
                Y2 = y*pixelsPerHistogram;
                [histogram, ~] = ComputeGradientHistogram(num_angles, patch_mag(X1:Y1,X2:Y2), patch_theta(X1:Y1,X2:Y2));
                feature = [feature,histogram];
            end
        end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    for i=1: size(gradient_magnitudes,1) * size(gradient_magnitudes,2)
        X = gradient_angles(i);
        cur_angle = 0;
        for j=1: num_bins
            if (X>=cur_angle && X<cur_angle+angle_step)
                histogram(j) = histogram(j) + gradient_magnitudes(i);
            end
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    direction = angles((histogram == max(max(histogram))));
